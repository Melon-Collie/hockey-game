...
# with their name keys, every zone resolves to one of the three slots a real
# design is built from, a forged model paints stock gear instead of crashing
# the paint seam, and row 0 of each list is still the pre-models look.
...
func test_row_zero_is_the_pre_models_look() -> void:
...
func test_forged_models_paint_stock_gear() -> void:
	...
	for model: int in [-1, -999, 99]:
		assert_eq(GearModelRegistry.skate_color(model, GearModelRegistry.SKATE_BOOT, accent),
