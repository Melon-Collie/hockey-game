...
func _run_rush(start: Vector3, vel: Vector3, secs: float = 3.0,
		trace: bool = false, profile: BotSkillProfile = null) -> Dictionary:
	if profile == null:
	...
	for t: int in int(secs / DT):
		...
		duel.step()
		shots += duel.releases.size() - before
	...
	return {"shots": shots, "min_dist": min_dist}
...
func test_every_angle_of_attack_produces_a_shot() -> void:
	...
	for tier: String in ["HARD", "NORMAL", "EASY"]:
		...
		for deg: float in [-85.0, -75.0, -60.0, -45.0, -25.0, 0.0, 25.0, 45.0, 60.0, 75.0, 85.0]:
			...
			gut.p("  %-6s angle %+5.0f°  →  shots=%d  min-dist=%.1f" % [
					tier, deg, r["shots"], r["min_dist"]])
			if r["shots"] <= 0:
...
func test_going_by_the_net_still_produces_a_shot() -> void:
	...
	for c: Array in cases:
		...
		gut.p("  %-22s shots=%d  min-dist=%.1f" % [c[0], r["shots"], r["min_dist"]])
		if r["shots"] <= 0:
			missed.append(c[0])
	assert_eq(missed.size(), 0,
			"a lateral 'going by' 1v1 must still produce a shot; missed: %s" % str(missed))
