...
# ArenaStands._row_facing_yaw — the heading every seat and every spectator in the
# bowl is placed at.
#
...
# A closed loop wound the way _sample_offset_path winds one: bottom edge (running
...
func test_every_seat_faces_the_ice() -> void:
	...
	for i: int in loop.size():
		var forward: Vector2 = _forward(ArenaStands._row_facing_yaw(loop, i))
...
func test_a_straight_rank_shares_one_heading() -> void:
	...
	var straight: float = ArenaStands._row_facing_yaw(loop, 4)
	for i: int in [5, 6, 7, 8]:
		assert_almost_eq(ArenaStands._row_facing_yaw(loop, i), straight, 1e-5,
...
func test_a_corner_seat_turns_with_the_arc() -> void:
	...
	var forward: Vector2 = _forward(ArenaStands._row_facing_yaw(loop, corner))
