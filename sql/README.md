...
|------|---------|-----------------|
| `career_stats.sql` | `career_stats` table, `career_totals` view, `recent_games_for()` RPC | `CareerStatsReporter`, `CareerStatsScreen` |
...
- **RLS** is on for all tables; the publishable (anon) key gets only the grants
  it needs (`bug_reports`/`network_sessions`: INSERT; `career_stats`: INSERT /
  SELECT / UPDATE). Reads for analysis go through the dashboard or a service-role
  key, which bypasses RLS. Never commit the service/secret key.
