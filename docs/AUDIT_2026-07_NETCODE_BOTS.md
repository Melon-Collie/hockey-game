...
`side_menu.gd` (`_on_join_pressed`) · `pause_menu.gd:244-256` · `hud.gd:1337-1350`. The join flow runs `reset()` + `start_client_lobby()` unconditionally with no "op in progress" flag; a Steam invite accepted mid-join-flow re-enters and races `SteamManager` state (the first join's one-shot signal connections get disconnected by the second `reset()`). (The host-side variant of this race was removed by the 2026-07 menu consolidation — hosting now starts offline and `attach_online` runs behind the lobby's visibility selector, which disables itself while the create is in flight.) Separately, the "Return to Free Play" / "Exit Game" confirms `await announce_match_end()` (0.5 s) while staying interactive, so a double-click fires two overlapping teardown chains and can double-`change_scene`. **Fix:** a single `_lobby_op_pending` gate; disable confirm buttons before the await.
