...
| `broadcast_interval_p95_ms` | ms | ≈ 16.7 (`STATE_RATE` 60 Hz target); >1.4× target = sagging | p95 gap between world-state broadcasts. Sustained high = host stalling or send path backed up. Omitted on client rows. |
| `input_queue_depth` | frames | 1–3 | The **client's** own echo of its pending-input depth — **folds 0 on host rows**, so a host row reading 0 here is structural, not starvation. For "client inputs buffered on the host" read `host_input_queue_depth` above, whose healthy value is the stamp lead in ticks (≈8–12 at the current 25 ms base plus the servo's extra) — NOT this row's 1–3. |
