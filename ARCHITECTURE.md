...
**Remote skaters render at the buffered past instant `render_time` with no forward projection to "present."** `RemoteController._interpolate`'s normal (non-extrapolating) branch outputs the hermite result at `render_time = estimated_host_time() − interpolation_delay` directly. It used to push the body forward by `interp_delay` toward present, which overshot on every direction change and snapped back when the next real sample arrived (`Guessing ahead` climbing, visible as rhythmic remote snap-back during fast play). The "interpolate in the past" model trades a little extra latency on *other* players' bodies for jitter-free motion; the local player is unaffected (still client-predicted). Forward extrapolation survives only in the `is_extrapolating` branch as the packet-loss fallback (render_time has outrun the newest buffered sample). Do not re-add a *steady* forward-advance to the interpolating branch. **One scoped exception (Lever B, `start_knockback_lead`):** a credited body check adds a brief 0→peak→0 `sin(t·π)` position pulse along the hit direction (`_knockback_lead_*`, ≤~0.22 m over ~120 ms) so the victim's knockback reads punchy instead of arriving `interp_delay` late. This is safe where a steady advance isn't because it's an event-scoped, fully-decaying transient — it cannot accumulate divergence — and it composes as a final additive offset after the interpolate/rejoin result. Do not generalize it into a continuous lead.
