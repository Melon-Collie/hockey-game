...
- **Test coverage gaps.** Remaining: `FileReplayDriver` / `GoalReplayDriver` (both `extends Node`, drive live scene actors — integration-test territory, poor headless ROI), and the role-dispatch inside `_state_off_puck` / `_state_chase_puck` (each slot routes to a full role behavior). The agent SM's snapshot-driven predicates, mouse/aim + wind-up geometry, dispatch guards/throttle, the **press-state handlers** (`_state_shoot_pressed` charge/release + stagger/front-pressure/lost-puck bails, `_state_quick_shot_pressed`, `_state_one_timer_pressed` hold/release/timeout, `_state_pass_pressed` quick + charged), and **`_state_carry`** (puck-loss bail + carrier reset, intent→State mapping for each fire kind, pre-aim-then-commit, the hysteresis hold against a carrier flip, and the pre-aim timeout) with their transitions are covered (`tests/unit/ai/`). The press states run off pre-set fields once entered, so they drive through `dispatch()` directly; `_state_carry` uses a stub `AIRoleCarrier` subclass (scripts an intent, spies on the `clear_intent`/`reset` lifecycle) to isolate the handler from the scorer. `PhaseCoordinator`, `SlotSwapCoordinator` host paths, and `decode_for_replay` are covered. Domain rules remain well-covered.
