...
- `GameRules.ICE_FRICTION = 0.05` — the AI/client puck-trajectory-prediction model of the host's ice glide (realistic puck-on-ice μ). The rink ice surface is a child `StaticBody3D` whose `PhysicsMaterial` is built in code by `HockeyRink` from its `ice_friction` export (also 0.05), so friction applies correctly; `ICE_FRICTION` must mirror that export. (There is no ice `.tres` — an orphaned `Physics/ice.tres` was removed after it misled `ICE_FRICTION` to 0.1 while the live ice ran 0.01.)
