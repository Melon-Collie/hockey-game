...
- CCD enabled on puck
- Puck mass 0.17 kg, collision radius 0.04 m / height 0.025 m (near-regulation 3"×1"; pickup/poke are center-based via `PICKUP_RADIUS`, so the collision size only affects physical bounces and fitting through goalie gaps, not corralling. `ice_height`/`PUCK_START_POS` track the cylinder half-height (0.0125) — update both if the height changes)
