...
- **Shot lag compensation:** *Origin rewind landed.* The host now rewinds the shooter's blade to `self_view_time(host_timestamp)` and fires the puck from that `blade_contact_world` (horizontal only — `release()`'s elevation-y carries through), composing with the pre-existing RTT/2 advance so the host trajectory starts where the client predicted and the three-zone reconcile stops correcting mid-flight (`on_remote_puck_release` / `_host_release_one_timer`). Goalie reaction back-date and goalie-position rewind were already in place. **Remaining nuance:** the goalie's shot-reaction (`_on_puck_released`) still reads the live-blade puck position at `release()` time, a tick before the origin is repositioned — so its threat geometry uses the un-rewound origin. Pre-existing and small (≤ RTT/2 of blade drift); revisit only if goalie save tuning calls for it.
