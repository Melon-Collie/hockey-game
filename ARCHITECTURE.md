...
- **Test coverage gaps.** Remaining: `FileReplayDriver` / `GoalReplayDriver` (both `extends Node`, drive live scene actors — integration-test territory, poor headless ROI), and `skater_agent_state_machine`'s two carrier-coupled handlers: `_state_carry` (runs `AIRoleCarrier.decide` — testing it headlessly needs a carrier intent stub) and the role-dispatch inside `_state_off_puck` / `_state_chase_puck` (each slot routes to a full role behavior). The agent SM's snapshot-driven predicates, mouse/aim + wind-up geometry, dispatch guards/throttle, and the **press-state handlers** (`_state_shoot_pressed` charge/release + stagger/front-pressure/lost-puck bails, `_state_quick_shot_pressed`, `_state_one_timer_pressed` hold/release/timeout, `_state_pass_pressed` quick + charged) with their transitions are covered (`tests/unit/ai/`) — those run off pre-set fields once entered, so they drive through `dispatch()` without the carrier. `PhaseCoordinator`, `SlotSwapCoordinator` host paths, and `decode_for_replay` are covered. Domain rules remain well-covered.
