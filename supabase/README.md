...
- **`SUPABASE_DB_URL`** — the **session pooler** URI, from the **Connect** button
  in the dashboard header, with `[YOUR-PASSWORD]` replaced. It looks like:
  ...
  This is **not** an API key. The publishable/secret API keys authenticate HTTP
  requests to PostgREST (that is what `SupabaseConfig.ANON_KEY` holds); this is
  the Postgres password, because `db push` speaks the database wire protocol.
  Neither substitutes for the other, and the secret key belongs in the repo even
  less than this does.

  The password is the one set at project creation; it is never shown again (the
  connection string carries a `[YOUR-PASSWORD]` placeholder). If it is lost,
  reset it under the **Database** section's settings —
  `dashboard/project/<ref>/database/settings`, which is NOT under Project
  Settings — and note that resetting invalidates anything else using it. Prefer
  a generated alphanumeric password:
