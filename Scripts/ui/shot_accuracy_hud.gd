...
# Code-built overlay for the shot-accuracy drill (mirrors PenaltyDrillHUD so
# there's no scene file to hand-edit). A compact top-right tracker shows the
# shot count, hits, and the called target spot; a centre flash calls HIT! /
# MISS after each attempt; a results card at the end offers Try Again or Exit.
# ShotAccuracyManager drives all of it.
...
# The called-spot line matches the bullseye's amber so the HUD text and the
# lit target on the net read as the same thing.
...
func _build_tracker() -> void:
	...
	# as PenaltyDrillHUD / TutorialHUD).
	...
	_hits_label.add_theme_font_size_override("font_size", 15)
	_hits_label.add_theme_color_override("font_color", MenuStyle.TEAL_HOVER)
	vbox.add_child(_hits_label)

	var hint := Label.new()
	hint.text = "Hit the lit target. Every release counts as a shot."
...
func set_progress(attempt_number: int, total: int, hits: int, target_name: String) -> void:
	_flash_label.visible = false
	_target_label.text = "▸ %s" % target_name
	_shot_label.text = "Shot %d / %d" % [attempt_number, total]
	_hits_label.text = "Hits: %d" % hits


func flash_result(hit: bool, hits: int, attempts_taken: int) -> void:
	_flash_label.text = "HIT!" if hit else "MISS"
	_flash_label.add_theme_color_override("font_color", _GREEN if hit else _RED)
	_flash_label.visible = true
	_hits_label.text = "Hits: %d / %d" % [hits, attempts_taken]


func show_results(hits: int, total: int) -> void:
	_flash_label.visible = false
	_results_heading.text = "%d / %d" % [hits, total]
	_results_sub.text = _verdict(hits, total)
