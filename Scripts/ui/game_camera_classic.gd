class_name GameCameraClassic
extends Camera3D

# The original Mitts camera, kept here as an opt-in alternative to the new
# cursor-driven design. Anchors at midpoint(player, puck), uses possession-
# aware slack-based zone bias, shakes on goals and big hits, and reads
# `camera_mode` (Orthographic / Top-Down Perspective / Tilted Perspective)
# rather than the new `tilt_angle` slider. Select via Options → Game →
# Camera → Style.
