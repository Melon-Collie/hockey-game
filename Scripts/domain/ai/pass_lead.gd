...
# build (AISkaterCaps) — its Acceleration (max_accel) caps how hard it can keep
...
static func lead(shooter_pos: Vector3, receiver: SkaterNetworkState,
	...
	# low-Acceleration receiver into ice it can't reach. Cap the along-travel accel
	# at the receiver's real max_accel.
