...
- **Physical** → `body_check_transfer` (how hard you DELIVER a hit), `body_check_brace_resistance` (hard to PUT DOWN — the active brace, inverted), and stamina (gentle drain/duration curve + steep low-end regen/recovery curve). Body checks read **two axes that compose, not double-count**: Size sets `weight` (mass; hard to move), Physical sets the transfer/brace coefficients (delivery + hard to put down). The grinder / motor stat *(+ visual shoulder/yoke bulk)*
