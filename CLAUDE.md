...
All tuning multipliers live as private constants on `PlayerAttributes`, accessed via named instance methods (`attrs.speed_mult()`, `attrs.shot_power_mult()`, `attrs.hands_blade_mult()`, `attrs.physical_check_mult()`, `attrs.height_mult()`, etc.) — **never index a `_*_MULTS` table outside that file**. Different effects need different spreads, and several attributes drive *multiple* tables: Shot has a charged-ceiling curve and a wrister-charge-effort curve; Physical splits stamina into separate drain (duration) and regen (recovery) curves; Hands has blade / carry / backhand. Visual scaling decouples from gameplay magnitudes (arm bulk runs ±40% asymmetric). Each attribute owns one visual tell: torso/head → Size, shoulders → Physical, thigh → Speed, calf → Agility, forearm → Hands, upper arm → Shot. To add a new "X scales Y" rule, see the doc-block at the top of `player_attributes.gd`.
